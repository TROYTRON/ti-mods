# Worked example: expand the ship-designer grid

The original worked example by dkoiman is retained for its object traversal, slot cloning, controller registration, patch mechanics, and screenshots. Read [the UI guide](IntroToUI.md) first. The 10-by-8 grid, private field names, hull fields, and hooks describe the original game build and have not been verified against the handbook baseline. The patch is not idempotent: repeating setup can duplicate slots or dictionary keys. Adapt it to the current screen lifecycle before use.
...
The layout of each hull type is defined in `TIShipHullTemplate.json`, specifically `shipModuleSlots` defines which slots the ship has, where they are displayed and what type of modules they can fit.
...
* change `constraintCount` to 11 (adding 1 column)
* add more `ShipModuleSlot` children
* organise the children in such a way that the grid correctly display the extra column
...
The original result added a column to the Titan's module layout. In the target build's `TIShipHullTemplate.json`, copy the current Titan record and extend `shipModuleSlots`: shift the existing nose slot from `x = 9` to `x = 10`, then add the intended slots in the new column. Keep `hullHardpoints` and `internalModules` consistent with the slots you add. The old example used 8 hull hardpoints and 10 internal modules; choose values for your actual design.

Use the [native array replacement policy](../docs/native-data.md#array-policies) when supplying the complete revised slot list. A sparse array under default index merging will retain unrelated existing entries. Preserve the target build's other hull fields instead of copying an old full Titan template. Test the ship model's mount objects and combat behavior separately from the designer UI.

![Expanded Titan in the original example](tutorial-files/larger_titan.png)
